...
public class MadgwickAHRS {
	...
	// freq
	private volatile double sampleFreq;
	// algorithm gain
	private volatile double beta;
	// quaternion of sensor frame relative to auxiliary frame
	private volatile double[] quaternion;		
	...
	public void MadgwickAHRSupdate(double gx, double gy, double gz, double ax, double ay, double az, double mx, double my, double mz)
	{
		double recipNorm;
		double s0, s1, s2, s3;
		...
		if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
			...
			recipNorm = 1.0f / (double)Math.sqrt(ax * ax + ay * ay + az * az);
			ax *= recipNorm;
			ay *= recipNorm;
			az *= recipNorm;   

			// Normalise magnetometer measurement
			recipNorm = 1.0f / (double)Math.sqrt(mx * mx + my * my + mz * mz);
			mx *= recipNorm;
			my *= recipNorm;
			mz *= recipNorm;
			...
			recipNorm = 1.0f / (double)Math.sqrt(s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3); // normalise step magnitude
			s0 *= recipNorm;
			s1 *= recipNorm;
			s2 *= recipNorm;
			s3 *= recipNorm;
		...
		recipNorm = 1.0f / (double)Math.sqrt(quaternion[0] * quaternion[0] + quaternion[1] * quaternion[1] + quaternion[2] * quaternion[2] + quaternion[3] * quaternion[3]);
	...
	public double[] getEulerAngles()
	{
		...
		angles[0] = (double) Math.atan2(2*quaternion[1]*quaternion[2]-2*quaternion[0]*quaternion[3],
				2*quaternion[0]*quaternion[0] + 2*quaternion[1]*quaternion[1] - 1);
		angles[1] = (double) (-1 * Math.asin(2*quaternion[1]*quaternion[3] + 2*quaternion[0]*quaternion[2]));
		angles[2] = (double) Math.atan2(2*quaternion[2]*quaternion[3]-2*quaternion[0]*quaternion[1],
	...
	/** Not working yet **/
	public double[] rotateVector3(double[] vector)
	{
		double[] res4D = rotateVector4(new double[] {0.0f, vector[0], vector[1], vector[2]} );
		return new double[] {res4D[1], res4D[2], res4D[3]};
	}
	
	/** Not working yet **/
	public double[] rotateVector4(double[] vector)
	{
		double length = (double) Math.sqrt(vector[0]*vector[0] + vector[1]*vector[1] + vector[2]*vector[2] + vector[3]*vector[3]);
		double[] quaternionInv = new double[4];
		quaternionInv[0] = quaternion[0];
		quaternionInv[1] = -quaternion[1];
		quaternionInv[2] = -quaternion[2];
		quaternionInv[3] = -quaternion[3];
		
		vector[0] /= length;
		vector[1] /= length;
		vector[2] /= length;
		vector[3] /= length;
		// rotated vector = q * vector * qInv
		double[] res = multiplyQuaternions(quaternion, multiplyQuaternions(vector, quaternionInv));
		res[0] *= length;
		res[1] *= length;
		res[2] *= length;
		res[3] *= length;
	...
	/** Not working yet **/
	public double[] multiplyQuaternions(double[] left, double[] right)
	{
		double[] res = new double[4];
		
		// (w1w2 - x1x2 - y1y2 - z1z2)
		res[0] = (left[0]*right[0] - left[1]*right[1] - left[2]*right[2] - left[3]*right[3]);
		// (w1x2 + x1w2 + y1z2 - z1y2)
		res[1] = (left[0]*right[1] - left[1]*right[0] - left[2]*right[3] - left[3]*right[2]);
		// (w1y2 - x1z2 + y1w2 + z1x2)
		res[2] = (left[0]*right[2] - left[1]*right[3] - left[2]*right[0] - left[3]*right[1]);
		// (w1z2 + x1y2 - y1x2 + z1w2
		res[3] = (left[0]*right[3] - left[1]*right[2] - left[2]*right[1] - left[3]*right[0]);
